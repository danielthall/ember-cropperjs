...
module('Integration | Component | image cropper call', function(hooks) {
  ...
  test('it calls the function on the object', async function(assert) {
    ...
    await render(hbs`{{image-cropper-call obj=obj func='func' args=args onResp=(action 'onResp')}}`);
  });
  ...
  test('it works with positionalParams', async function(assert) {
    ...
    await render(hbs`{{image-cropper-call 'func' 'a' 1 'z' obj=obj onResp=(action 'onResp')}}`);
  });
});
